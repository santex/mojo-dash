...
d3.select("#paste-go").on("click", function() {
  ...
  if (d3.select("#paste-save").property("checked")) {
    d3.text("save").post(d3.select("#paste").property("value"), function(error, filename) {
      if (!error) url({source: state.source = filename});
    });
  }
...
function getURL(url, callback) {
  if (/^@/.test(url)) {
    ...
    iframe.attr("src", "http://quantup.com/?"
...
function proxy(url, callback) {
 ...
 getURL("http://localhost/docs/php/mojo.php?target="+encodeURIComponent(url)+"&source=" + encodeURIComponent(document.location), callback);
...
function change() {
  ...
  if (state.source !== last && state.source) {
    ...
    proxy(state.source, processText);
